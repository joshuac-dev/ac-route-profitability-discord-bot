    ...
    builder.addSubcommand(sub => sub
        ...
        .addIntegerOption(opt =>
            opt.setName('min_economy')
               .setDescription('Minimum direct economy demand required to consider a route')
               .setMinValue(0)
               .setRequired(false)
        )
    );
...
export async function execute(interaction) {
    ...
    if (minEconomyDemand > 0) {
        console.log(`[RUN] Minimum direct economy demand filter: ${minEconomyDemand}`);
    }
    ...
    try {
        const results = await runAnalysis(
            ...
            testLimit,
            onProgress,
            { minEconomyDemand } // NEW: pass threshold
        );
